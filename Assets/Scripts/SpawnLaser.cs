...
public class SpawnLaser : MonoBehaviour
{
    ...
    void SpawnLaserWave(int laserToSpawn)
    {
        for (int i = 0; i < laserToSpawn; i++)
        {
            ...
            var LaserGO = Instantiate(lasersPrefabs[laserIndex], new Vector3(0, 0, 10 + i * 5), lasersPrefabs[laserIndex].transform.rotation);
            LaserGO.GetComponent<MoveTowardPlayer>().speed = laserToSpawn * 2.5f;
        }
